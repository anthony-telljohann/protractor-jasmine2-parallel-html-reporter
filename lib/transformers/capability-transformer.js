...
export default {
  toReporter(capability) {
    return reporterBuilder.setSavePath(path.join(temporaryDirectoryService.get(), capability.get('platform'), capability.get('browserName'), capability.get('version'))).build()
  }
}
